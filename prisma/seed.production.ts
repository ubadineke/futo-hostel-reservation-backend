...
type RoomSeed = { name: string; capacity: number; bedsTotal: number };
type HostelSeed = {
  id: string;
  name: string;
  code: string;
...
// Catalog data only — no fake occupancy. Values reproduced verbatim from
// BACKEND-README.md §8 / app/lib/core/demo/hostel_data.dart; REQUIREMENTS.md §9
// notes gender/price/blurb are representative, not officially published by
// FUTO — confirm with the real Hostel Officer and adjust via the admin API
// (PATCH /admin/hostels/:id, /admin/rooms/:id) once real figures are known.
const HOSTELS: HostelSeed[] = [
  {
    ...
    rooms: [
      { name: '8-bed room', capacity: 8, bedsTotal: 48 },
      { name: '10-bed room', capacity: 10, bedsTotal: 40 },
    ],
  },
  {
    ...
    rooms: [
      { name: '8-bed room', capacity: 8, bedsTotal: 48 },
      { name: '10-bed room', capacity: 10, bedsTotal: 50 },
    ],
  },
  {
    ...
    rooms: [
      { name: '6-bed room', capacity: 6, bedsTotal: 36 },
      { name: '8-bed room', capacity: 8, bedsTotal: 48 },
    ],
  },
  {
    ...
    rooms: [
      { name: '6-bed room', capacity: 6, bedsTotal: 36 },
      { name: '8-bed room', capacity: 8, bedsTotal: 32 },
    ],
  },
  {
    ...
    rooms: [{ name: '8-bed room', capacity: 8, bedsTotal: 64 }],
  },
  {
    ...
    rooms: [{ name: '4-bed room (en-suite)', capacity: 4, bedsTotal: 80 }],
  },
  {
    ...
    rooms: [
      { name: '4-bed room', capacity: 4, bedsTotal: 64 },
      { name: '3-bed room', capacity: 3, bedsTotal: 36 },
    ],
  },
  {
    ...
    rooms: [
      { name: '2-bed room', capacity: 2, bedsTotal: 40 },
      { name: '1-bed studio', capacity: 1, bedsTotal: 12 },
    ],
  },
];
    ...
    for (const h of HOSTELS) {
      ...
      for (const r of h.rooms) {
        const room = await prisma.room.create({
          data: { hostelId: h.id, name: r.name, capacity: r.capacity },
        });
        await prisma.bed.createMany({
          data: Array.from({ length: r.bedsTotal }, (_, i) => ({ roomId: room.id, number: i + 1 })),
        });
      }
    }
    console.log(`Seeded ${HOSTELS.length} hostels with their room types and beds (all available).`);
  }
